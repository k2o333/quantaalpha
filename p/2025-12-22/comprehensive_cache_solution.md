...
本文档描述了为aspipe_v4项目实现的综合缓存解决方案。该方案完全基于现有架构进行优化，避免重复实现和架构混乱，充分利用现有的`data_storage.py`模块和配置系统。

## 当前缓存实现分析

### 现有缓存功能

1. **数据存储模块** (`app/data_storage.py`)
   - `get_cache_path()` - 生成缓存路径
   - `is_data_cached()` - 检查数据是否已缓存
   - `is_data_fresh()` - 检查缓存是否新鲜
   - `save_to_parquet()` - 保存数据到Parquet文件
   - `load_from_parquet()` - 从Parquet文件加载数据

2. **配置适配器** (`app/config_adapter.py`)
   - `get_cache_settings()` - 获取接口缓存设置

### 存在的问题

1. **缓存功能分散** - 缓存逻辑分布在多个模块中
2. **重复代码** - 在不同地方重复实现相似的缓存检查逻辑
3. **利用率不足** - 当前调度器和策略中未充分使用缓存功能
4. **缺乏统一接口** - 没有一个统一的缓存访问接口
5. **接口不一致** - 缓存相关的函数命名和功能不统一
...
def get_interface_cache_path(data_type: str, **kwargs) -> str:
    ...
    Returns:
        缓存文件路径
    """
    from data_storage import DATA_DIR
...
3. **财务数据**（income, balancesheet, cashflow等）：TTL 24-48小时，按报告期缓存
...
7. **统一接口** - 通过`get_cached_or_download_data`函数提供统一的缓存访问接口，减少重复代码
8. **智能缓存** - 支持基于参数和接口类型的智能缓存策略
9. **可配置缓存** - 支持不同接口类型的缓存策略配置
10. **错误容错** - 在缓存功能出错时能够优雅降级

这个综合的缓存解决方案完全基于现有系统架构，避免了重复实现问题，同时提供了有效的缓存功能和统一的访问接口。