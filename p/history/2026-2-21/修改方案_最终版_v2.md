...
2. pagination_executor.execute() -> PaginationComposer.compose()
...
2. pagination_executor.execute() -> PaginationComposer.compose()
   - 检测到 pagination_mode == "reverse_date_range" 且 is_date_anchor_interface == True
   - 调用 _apply_date_anchor_range() 方法
...
| 模式 | 调用路径 | 是否经过 params_builder |
|------|---------|------------------------|
| 普通下载 | main.py → params_builder → pagination_executor | ✅ 经过 |
| 更新模式 | update_manager → pagination_executor → compose() | ❌ 不经过 |

**结论**：必须在 `pagination.py` 中也添加处理逻辑，才能修复更新模式。
...
### 修改点 7: pagination.py - compose 方法（关键修改）

**文件**: `app4/core/pagination.py`

**位置**: 第 77-92 行（`compose` 方法）
...
- **创建时间**: 2026-02-22
- **版本**: v2（补充遗漏的 pagination.py 修改）
- **适用版本**: aspipe_v4
- **修改类型**: Bug 修复
- **修改文件**: 
  - `app4/core/params_builder.py`（新增约 60 行）
  - `app4/core/pagination.py`（新增约 60 行）
