...
- **PreloadTradeCal**: 预加载交易日历，使用三级缓存策略（内存 → Data目录 → API），特别处理Data目录的parquet dataset读取

### 2. 并发控制机制
- **TaskScheduler**: 基于 ThreadPoolExecutor，默认最大工作线程数 4
- **RateLimiter**: 令牌桶算法，时间窗口60秒，支持令牌补充逻辑
- **随机延迟**: 0.05-0.5 秒，避免所有线程同时唤醒
- **任务分批**: 每批 100 个任务，避免内存溢出
- **速率限制**: 基础延迟 sleep_time = time_window / rate_limit，额外增加0-10%随机抖动
...
| 检查点5 | 增量下载决策 | date_range 模式 | 返回 skip/partial/full（智能阈值30%） |
| 检查点6 | 批次内去重 | 数据处理后 | 全部重复则跳过批次 |
| 检查点7 | 接口配置去重 | 保存前 | 基于主键过滤新记录 |

**缓存策略**:
- 使用线程锁保护缓存读写
- 缓存键格式：`(interface, tuple(sorted_params))`
- 支持覆盖率结果缓存和lazy loading
- 覆盖率决策逻辑：coverage ≥ 95% → skip；coverage > 30% → partial；coverage ≤ 30% → full
...
**去重策略**:
- 基于主键去重，按_update_time排序保留最新数据
- 批次内去重检查，全部重复则跳过批次

**异步保存**:
- 批次大小：10000 条
- 文件格式：Parquet Dataset 模式
- 文件名格式：`{interface}_{date_range}_{timestamp}_{uuid}.parquet`（date_range从数据中提取）
- 原子写入：临时文件 + 重命名
- 后台线程处理，不阻塞下载流程
- 支持数据追加操作
...
- 指数退避策略
- 初始延迟 2 秒，退避因子 2
- 针对状态码 [429, 500, 502, 503, 504] 重试
- 连接池优化：10个连接池，最大20连接

**API 重试**:
- 检测频率限制错误（包含"频繁"、"limit"关键词的响应消息）
- 执行退避重试：base_delay * (2^attempt) + random.uniform(0, 2)
- 随机延迟 0-2 秒，避免多个线程同时重试
- 捕获异常不影响其他任务
...
- 单次数据量超过 6000 条告警
- 重试次数超过 2 次告警
- 数据量接近 API 限制告警（阈值 5800 条，建议减小窗口大小）

**异常处理**:
- 单股票下载异常不影响其他股票
- 单批次异常不影响其他批次
- 记录详细错误日志和调试信息
- 超时配置：(连接超时 10秒, 读取超时 60秒)

## 性能优化特性

1. **多级缓存**: 内存 + 磁盘 + API，减少重复请求
2. **智能增量下载**: 根据覆盖率自动决策下载策略，智能阈值30%
3. **文件名过滤**: 根据日期范围过滤文件，减少读取量
4. **确定性去重**: 基于主键去重，保留最新数据
5. **异步存储**: 后台线程写入，不阻塞下载流程
6. **批处理**: 每 10000 条数据保存一次，避免小文件爆炸
7. **原子写入**: 临时文件 + 重命名，确保数据完整性
8. **性能监控**: 实时跟踪请求时间、数据量、重试次数
9. **连接池优化**: 连接池大小10，最大连接数20
10. **交易日历优化**: 预加载全局交易日历，支持API和本地数据回退
...
request:
  ...
  retry_backoff: 2
  jitter_min: 0.05
  jitter_max: 0.5  # 增加到0.5秒
  rate_limit: 500  # 基础速率限制，接口级别可覆盖
  timeout: 60

storage:
  base_dir: "../data"  # 相对路径，实际运行时相对于app4目录
  format: "parquet"
  batch_size: 10000  # 优化批次大小
```
...
permissions:
  min_points: 0
  rate_limit: 120  # 接口级别速率限制，覆盖全局默认值
  query_limit: 10000
...
| 组件 | 文件路径 | 主要方法 |
|------|----------|----------|
| 主入口 | `app4/main.py` | `main()`, `process_and_save_data()`, `preload_global_trade_calendar()` |
| 下载器 | `app4/core/downloader.py` | `download()`, `download_single_stock()`, `_execute_date_range_pagination()` |
| 覆盖率管理 | `app4/core/coverage_manager.py` | `should_skip()`, `get_missing_date_ranges()`, `_check_range_coverage()` |
| 存储管理 | `app4/core/storage.py` | `read_interface_data()`, `save_data()` |
| 数据处理 | `app4/core/processor.py` | `process_data()`, `validate_data()` |
| 调度器 | `app4/core/scheduler.py` | `submit_task()`, `submit_tasks()`, `RateLimiter` |
| 配置加载 | `app4/core/config_loader.py` | `load_global_config()`, `get_interface_config()` |
| 性能监控 | `app4/core/downloader.py` | `PerformanceMonitor.record_metric()`, `check_alerts()` |

## 总结

app4 系统的单接口下载流程具有以下特点：

1. **配置驱动**: 基于 YAML 配置，零代码添加新接口
2. **多层防护**: 7 个重复检查点，确保数据唯一性，智能阈值优化
3. **智能决策**: 自动选择最佳下载策略（完整/部分/跳过），支持增量下载
4. **高性能**: 多级缓存、异步存储、批处理优化、连接池优化
5. **高可靠**: 原子写入、错误重试、异常隔离、性能监控告警
6. **易维护**: 模块化设计、清晰的职责分离、线程安全
7. **零代码扩展**: 新增接口只需创建 YAML 配置文件，无需编写代码

这种设计确保了系统在高并发、大数据量场景下的稳定性和性能，同时保持了极高的灵活性和可维护性。系统通过配置驱动的方式实现了真正的零代码扩展能力。