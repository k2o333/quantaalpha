...
彻底去掉 cache 方案，每次下载前把已有数据的 primary key 加载到内存，下载时对比去重，然后写入 data 目录。同时解决现有方案中的性能瓶颈和逻辑冲突，并**修复 Parquet 追加写导致的文件损坏问题**。
...
### 1. `core/storage.py` - 优化写入逻辑（修正后）

采用 Parquet Dataset 模式，避免文件损坏：
...
1. **数据一致性**: Dataset 模式需确保主键唯一性，建议在读取时增加全局去重检查
2. **小文件问题**: 高频写入可能产生大量小文件，建议定期合并（Compaction）
3. **兼容性**: Dataset 读取需要 polars >= 0.19.0，确保依赖版本正确
4. **存储空间**: 临时文件可能占用更多空间，需监控磁盘使用

---

## 后续建议

1. **监控**: 添加性能监控，跟踪内存、IO 使用和小文件数量
2. **Compaction**: 实现定期合并小文件的任务，优化读取性能
3. **事务支持**: 考虑写入临时文件后原子移动，确保写入原子性
4. **文档**: 更新相关文档，说明 Dataset 模式的使用方法
5. **测试**: 增加 Dataset 模式的集成测试，确保读写正确性