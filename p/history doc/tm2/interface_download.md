...
本文档描述了针对以下5个必须传递股票代码参数的接口实现全历史下载功能的方案，基于现有代码库的机制：
...
--tscode-historical # 下载全历史数据而非指定日期范围（仅适用于指定接口）
```
...
```python
# 在 interfaces/holders_data.py 中添加功能
...
为`top10_holders`、`pledge_detail`、`fina_audit`、`pro_bar`等接口实现下载功能。
...
```python
# 在 enhanced_download_config.py 中为这些接口添加配置
{
...
#### 6.1 修改主下载器
在ScoreBasedDownloader或TuShareDownloader中集成功能：
...
```bash
# 仅下载holders相关数据（全历史）
python app/main.py --holders-data --tscode-historical

# 仅下载pro_bar（全历史）
python app/main.py --pro-bar-only --tscode-historical

# 同时下载holders和pro_bar（全历史）
python app/main.py --holders-data --pro-bar-only --tscode-historical

# 带自定义参数下载
python app/main.py --holders-data --tscode-historical --start-date 20200101
```
...
该方案基于现有代码库的机制，为特定接口实现全历史下载功能，提高数据下载效率。