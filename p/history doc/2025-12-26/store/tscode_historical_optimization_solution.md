...
def _execute_tscode_download(self, **kwargs) -> pd.DataFrame:
    ...
    for ts_code in ts_codes:
        download_tasks.append((ts_code, interface_name))

    # 使用线程池并行下载
    max_workers = min(4, len(ts_codes))  # 最多4个线程
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_tscode):
            ts_code = future_to_tscode[future]
            try:
                df = future.result()
                if df is not None and not df.empty:
                    all_data.append(df)
            except Exception as e:
                self.logger.warning(f"下载 {interface_name} - {ts_code} 失败: {e}")
                continue
...
def _download_single_tscode(self, ts_code: str, interface_name: str) -> pd.DataFrame:
    """
    下载单个股票的指定接口数据
    支持缓存检查
    ...
    Returns:
        下载的数据
    """
    # 检查缓存（与日期范围模式完全相同的缓存机制）
    cache_kwargs = {'ts_code': ts_code}
    ...
    if cache_settings['enabled']:
        if is_interface_data_cached(
            ...
            **cache_kwargs
            ...
            self.logger.info(f"使用缓存数据: {interface_name} - {ts_code}")
...
4. **添加 `_download_single_tscode()` 方法**
   - 检查缓存（与日期范围模式相同）
   - 下载新数据
   - 保存缓存（与日期范围模式相同）
...
| `_get_stock_list()` | 获取股票列表（带缓存） | 新增 |
| `_execute_tscode_download()` | 执行 tscode 接口下载 | 类似 `_execute_daily_download()` |
| `_download_single_tscode()` | 下载单个股票数据 | 类似 `_execute_daily_download()` 中的单日下载 |
