...
def test_express_vip_download():
    """
    测试express_vip接口的下载能力，验证能否获取超过10000条数据
    """
    print("=" * 60)
    print("开始测试express_vip接口大数据量下载...")
    print("=" * 60)
    ...
    if downloader.current_points >= 5000:
        ...
        if all_data:
            ...
            if unique_records >= 10000:
                print(f"✅ VIP接口测试通过! express接口成功获取超过10000条记录 ({unique_records}条)")
                return True
            else:
...
def test_express_fallback():
    """
    测试积分不足时的降级方案（逐股票下载）
    """
    print("\n" + "=" * 60)
    print("开始测试express降级方案...")
    print("=" * 60)
    ...
    if downloader.current_points < 5000:
        ...
        try:
            df = downloader.download_express(period='20231231')
            if df is not None and not df.empty:
...
def test_express_data_integrity():
    """
    测试数据完整性和格式正确性
    """
    print("\n" + "=" * 60)
    print("开始测试express数据完整性与格式...")
    print("=" * 60)
    ...
    try:
        ...
        if df is not None and not df.empty:
            ...
            # 检查是否有关键列
            expected_columns = ['ts_code', 'ann_date', 'end_date', 'revenue', 'profit']
...
def main():
    ...
    # 执行VIP下载测试
    results['vip_download'] = test_express_vip_download()

    # 执行降级方案测试
    results['fallback_download'] = test_express_fallback()

    # 执行数据完整性测试
    results['data_integrity'] = test_express_data_integrity()

    # 输出最终结果
    print("\n" + "=" * 60)
    print("📊 Express大数据量下载测试结果:")
