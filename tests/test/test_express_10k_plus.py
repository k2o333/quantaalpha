...
def test_express_10k_plus():
    """
    测试express接口下载10000+条数据
    """
    print("=" * 60)
    print("开始测试express接口10000+条数据下载...")
    print("=" * 60)
    ...
    if downloader.current_points >= 5000:
        ...
        if total_records >= 10000:
            print(f"✅ express接口测试通过！获取了 {total_records} 条记录")
            return True
        else:
            if all_data:
                ...
                if unique_records >= 10000:
                    print(f"✅ express接口测试通过！去重后获取了 {unique_records} 条记录")
                    return True
                else:
                    print(f"❌ express接口测试未通过！仅获取了 {unique_records} 条记录")
                    return False
            else:
                print("❌ express接口测试未通过！没有获取到任何数据")
                return False
    else:
...
def main():
    ...
    主测试函数
    """
    print("Express接口10000+条数据下载验证")
    print(f"测试时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    # 配置日志
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

    success = test_express_10k_plus()

    print("\n" + "=" * 60)
    if success:
        print("🎉 Express接口10000+条数据下载测试通过！")
    else:
        print("💥 Express接口10000+条数据下载测试未通过！")
    print("=" * 60)
