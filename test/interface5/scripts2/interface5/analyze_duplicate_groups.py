...
def analyze_duplicate_groups(result_file: str) -> Dict[str, Any]:
    """
    分析同一主键组的多个记录
        ...
        if 'sample_preserved_records' in result and result['sample_preserved_records']:
            ...
            for pk_values, records in pk_groups.items():
                if len(records) >= 2:
                    ...
                    if field_diffs:
                        analysis[analysis_key]['duplicate_groups'].append({
...
def generate_duplicate_groups_report(analysis: Dict[str, Any]) -> str:
    """生成重复组报告"""
    report = []
    report.append("=" * 80)
    report.append("同一主键组的多个记录字段差异分析")
    report.append("=" * 80)
    ...
    total_groups = sum(len(data['duplicate_groups']) for data in analysis.values())
    
    if total_groups == 0:
        report.append("\n⚠️  未找到同一主键组的多个记录")
        report.append("\n这说明：")
        report.append("  1. 虽然主键去重比全字段去重重除了更多记录")
        report.append("  2. 但这些被去除的记录的主键都是不同的")
        report.append("  3. 全字段去重保留的记录是因为它们的全字段值不同")
        report.append("  4. 主键去重丢弃的记录是因为它们的主键与其他记录相同")
        report.append("")
        report.append("  结论：主键设置是合理的，API返回的数据确实存在完全重复的记录")
    else:
        report.append(f"\n共找到 {total_groups} 组同一主键的多个记录")
        ...
        for key, data in sorted(analysis.items(),
            ...
            report.append(f"发现 {len(data['duplicate_groups'])} 组同一主键的多个记录")
            ...
            for i, group in enumerate(data['duplicate_groups'], 1):
                ...
                sorted_fields = sorted(group['field_differences'].items(),
                ...
                # 显示示例记录
                report.append(f"\n示例记录 (前{len(group['sample_records'])}条):")
                for j, record in enumerate(group['sample_records'], 1):
                    report.append(f"\n  记录{j}:")
...
def main():
    ...
    # 分析重复组
    analysis = analyze_duplicate_groups(result_file)
    
    # 生成报告
    report = generate_duplicate_groups_report(analysis)
    print(report)
    
    # 保存报告
    output_file = Path("/home/quan/testdata/aspipe_v4/p/interface5/duplicate_groups_report.txt")
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(report)
    print(f"\n报告已保存到: {output_file}")
    
    # 保存JSON格式的详细数据
    json_output_file = Path("/home/quan/testdata/aspipe_v4/p/interface5/duplicate_groups.json")
