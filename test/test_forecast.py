...
"""
Forecast接口测试脚本
用于测试forecast接口的下载功能，确保能够获取超过10000条数据
"""
...
def test_forecast_data():
    ...
    if all_data:
        ...
        if unique_records >= 10000:
            print(f"✅ 测试通过! forecast接口成功获取超过10000条记录 ({unique_records}条)")
            return True
        else:
            print(f"❌ 测试未通过! forecast接口获取记录不足 ({unique_records}条)")
            return False
    else:
        print("❌ 没有获取到任何数据")
        return False

def test_forecast_with_pagination():
    """
    测试使用分页下载forecast数据以达到超过10000条记录
    """
    print("\n" + "=" * 60)
    print("开始测试forecast接口分页下载...")
    print("=" * 60)
...
def main():
    ...
    # 首先尝试普通的多周期测试
    if test_forecast_data():
        success = True
    else:
        # 如果普通测试未通过，尝试分页测试
        success = test_forecast_with_pagination()

    # 输出最终结果
    print("\n" + "=" * 60)
    if success:
        print("🎉 Forecast接口测试最终通过!")
    else:
        print("💥 Forecast接口测试未通过!")
    print("=" * 60)
