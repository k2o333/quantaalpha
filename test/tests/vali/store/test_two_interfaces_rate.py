...
"""
测试两个接口按天下载数据的极限速率
针对 daily 和 daily_basic 接口的速率测试
"""
...
def get_recent_trading_days(downloader, count=5):
    """
    获取最近的交易日列表
    """
    try:
...
def test_daily_rate_multiple_days():
    """
    测试daily接口按多个交易日下载的速率
    """
    print("\n=== 测试 daily 接口按天下载速率 ===")
    downloader = TuShareDownloader()

    # 获取最近的交易日
    trading_days = get_recent_trading_days(downloader, 3)
    ...
    for i, trade_date in enumerate(trading_days):
        ...
        try:
            ...
            if download_time > 0:
                print(f"  - 平均速率: {record_count/download_time:.0f} records/秒")
...
def test_daily_basic_rate_multiple_days():
    """
    测试daily_basic接口按多个交易日下载的速率
    """
...
def main():
    ...
    # 测试daily接口
    daily_records, daily_time = test_daily_rate_multiple_days()

    # 测试daily_basic接口
    daily_basic_records, daily_basic_time = test_daily_basic_rate_multiple_days()
