...
- 主要 `logger.warning()` 调用使用 f-string 格式，不再使用 `%s` 多参数格式
- 同类隐患位置建议一并修复
...
- [ ] **T01: 修复 client.py:69-74 的 logger.warning 调用（主要触发点）** `est:15m`
...
- [ ] **T02: 修复 client.py:667 的 logger.warning 调用（同类隐患）** `est:10m`
...
- [ ] **T03: 修复 universe.py:111 的 logger.warning 调用（同类隐患）** `est:10m`
  - Why: 同样使用 `%s` 格式，同类隐患
