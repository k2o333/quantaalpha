...
所有 AI Agents 和开发行为在动手修改代码前必须首要遵循以下两个核心治理文档：

1. **开发工作流规则**：阅读并遵守 [development-workflow.md](./development-workflow.md) 限定边界与流程管理，包括：
   - 何时可于 `main` 直接开发，何时必须启动隔离分支（`main` 保障稳定机制）
   - AI 的可自动实施边界及需要人工审查合并的权限边界
   - 每项任务的原子化提交（Atomic Commit）及本地测试检验规范
2. **文档规范体系**：具体如何增修维护文本内容，参见 [doc-rules.md](./doc-rules.md)，包括：
   - 文档类型定义划分（如草稿 Draft / 模块 / 变更 Change Doc）及其管理目录存放位置定规
   - 保证只有真正当前运行逻辑真相变动时才改写权威文档（Source of Truth），及 AI 文档使用约限
