...
if False:  # SOTA_factor is not None and not SOTA_factor.empty:
    ...
    combined_factors = new_factors  # 实际只走这个分支
```

**实际行为**：

- 系统只处理当前轮次生成的新因子（`new_factors`）
- `deduplicate_new_factors()` 方法虽然存在，但**永远不会被调用**
- 最终写入 parquet 的只有 `new_factors`，不会与历史 SOTA 因子合并
