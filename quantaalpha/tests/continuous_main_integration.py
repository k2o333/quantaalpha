...
class TestCycleBudgetAndAdaptiveSleep:
    ...
    def test_adaptive_sleep_skips_sleep_when_cycle_exceeds_interval(self, tmp_path):
        ...
        if sleep_times:
            assert sleep_times[0] < config.data_check_interval_seconds, f"Expected adaptive sleep < {config.data_check_interval_seconds}s, got {sleep_times[0]}s (adaptive sleep not implemented)"
    ...
    def test_budget_check_within_run_once_cycle(self, tmp_path):
        ...
        assert result.get("mining", {}).get("added") == 0 or result.get("mining", {}).get("added") == 1, "Expected mining to be skipped or limited when budget exhausted"
    ...
    def test_budget_enforcement_skips_mining_when_exhausted(self, tmp_path):
        ...
        assert result.get("mining", {}).get("added") == 0, f"Budget enforcement failed: mining.added={result.get('mining', {}).get('added')}, expected 0 (mining should be skipped when budget exhausted during revalidation)"
    ...
    def test_budget_enforcement_result_includes_budget_exhausted_flag(self, tmp_path):
        ...
        orchestrator._orchestrator.run_revalidation_cycle = slow_revalidation
        orchestrator._orchestrator.run_mining_cycle = MagicMock(return_value=MagicMock(factors_generated=3, factors_validated=2, factors_added=1, factor_ids=["gen_1"], errors=[], duration_seconds=0.5))

        result = orchestrator.run_once_cycle()

        # Budget exhaustion state must be in result for runtime feedback
        assert "budget_exhausted" in result, "budget_exhausted field missing from result - runtime doesn't know budget state"
        assert result["budget_exhausted"] is True, f"budget_exhausted should be True when budget exceeded, got {result.get('budget_exhausted')}"
...
class TestDataUpdateFieldsPassthrough:
    ...
    def test_run_once_cycle_passes_freshness_delta_from_update_result(self, tmp_path):
        ...
        with patch.object(ContinuousOrchestrator, "_create_bridge") as mock_create:
            ...
            assert "freshness_delta" in result["data_update"], "freshness_delta missing from data_update - run_update result not passed through"
            assert result["data_update"]["freshness_delta"].get("daily") == -1, "freshness_delta value not preserved from run_update result"
    ...
    def test_run_once_cycle_passes_unchanged_after_update_from_update_result(self, tmp_path):
        ...
        with patch.object(ContinuousOrchestrator, "_create_bridge") as mock_create:
            ...
            assert "unchanged_after_update" in result["data_update"], "unchanged_after_update missing from data_update - run_update result not passed through"
            assert "daily" in result["data_update"]["unchanged_after_update"], "unchanged_after_update value not preserved from run_update result"
    ...
    def test_run_once_cycle_preserves_advanced_interfaces_in_summary(self, tmp_path):
        ...
        with patch.object(ContinuousOrchestrator, "_create_bridge") as mock_create:
            ...
            assert "advanced_interfaces" in result["data_update"], "advanced_interfaces missing from data_update - upstream value not preserved"
            assert result["data_update"]["advanced_interfaces"] == ["adj"], "advanced_interfaces value not preserved from run_update result"
