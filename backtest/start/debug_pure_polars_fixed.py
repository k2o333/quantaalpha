...
def calculate_alpha101_factor_debug(df: pl.DataFrame, debug_dir: str):
    ...
    # 保存Step 2结果
    df.write_csv(f"{debug_dir}/step2_rolling_corr.csv")
    ...
    # 统计alpha3分布
    print(f"\n  Alpha3统计:")
    print(f"    非NaN数量: {df['alpha3'].is_not_nan().sum()}")
    print(f"    NaN数量: {df['alpha3'].is_nan().sum()}")
    print(f"    最小值: {df['alpha3'].min()}")
    print(f"    最大值: {df['alpha3'].max()}")
    print(f"    均值: {df['alpha3'].mean()}")
...
def main():
    ...
    START_DATE = "20240101"
    END_DATE = "20240131"
    DEBUG_DIR = "/home/quan/testdata/aspipe_v4/debug_pure_polars_fixed"
