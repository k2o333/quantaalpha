#!/usr/bin/env python3
"""使用vnpy的Alpha101因子进行回测 - Polars引擎"""
import sys
...
class SimpleBacktestEngine:
    ...
    def calculate_alpha101_factor(self, df: pl.DataFrame) -> pl.DataFrame:
        """计算Alpha101因子 (Alpha3: ts_corr(cs_rank(open), cs_rank(vol), 10) * -1)"""
        # 先计算截面排名
        df = df.with_columns([
        ...
        # 使用groupby + map_groups转换为pandas计算rolling corr
        ...
        print("计算Alpha101因子...")
        df = df.group_by("ts_code", maintain_order=True).map_groups(calc_rolling_corr)
        return df
...
def main():
    ...
    print("="*50)
    daily_df.write_csv("/home/quan/testdata/aspipe_v4/backtest_result.csv")
    print("\n详细结果已保存到: /home/quan/testdata/aspipe_v4/backtest_result.csv")
